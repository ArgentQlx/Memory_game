...
amountBtns.forEach(e => {
    e.addEventListener('click', () => {
        ...
        amountHint.textContent = `Number: ${amount}`;
    });
})
...
startGameBtn.addEventListener('mouseover', () => {
    if (startGameBtn.disabled) {startGameBtn.classList.remove('hovered')}
    else {
    ...
    e.addEventListener('click', () => {
        if (parseInt(e.textContent) === sequince[globalSequinceCounter]) {
            ...
            setTimeout(() => {
                ...
                if (globalSequinceCounter === sequince.length) {endGame(); correct()};
            }, 500);
        } else {
